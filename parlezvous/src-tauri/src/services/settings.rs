...
pub fn get_settings(db: Arc<Mutex<Connection>>) -> Result<AppSettings, String> {
    ...
    conn.query_row(sql, [], |row| {
        Ok(AppSettings {
            ...
            ai_provider: row.get(3).unwrap_or_else(|_| "ollama".into()),
            ai_base_url: row.get(4).unwrap_or_else(|_| "http://localhost:11434/v1".into()),
            ...
            litert_max_tokens: row.get(10).unwrap_or(5000),
