...
impl OpenAiCompatibleAdapter {
    ...
    pub(crate) async fn generate_json<T: DeserializeOwned>(
        ...
        for attempt in 1..=3 {
            match self.chat(model, messages.clone(), max_tokens, timeout_secs).await {
                Ok(content) => {
    ...
    pub(crate) async fn embedding(&self, text: String, model: String) -> Result<Vec<f64>, String> {
        let body = json!({ "model": model, "input": text, "encoding_format": "float", "dimensions": 768 });
        ...
        if embedding.len() != 768 {
            return Err(format!("Embedding model returned {} dimensions; textbook RAG currently requires 768", embedding.len()));
        }
        Ok(embedding)
    }
}
    ...
    async fn speaks_openai_compatible_models_chat_and_embeddings() {
        ...
        thread::spawn(move || {
            for _ in 0..3 {
                ...
                let (status, body) = if request.starts_with("GET /v1/models ") {
                    ...
                    assert!(request.contains("\"dimensions\":768"));
                    ("200 OK", serde_json::json!({"data":[{"embedding": vec![0.25; 768]}]}).to_string())
                } else {
        ...
        connection.execute_batch(&format!(r#"
            CREATE TABLE settings (
                id INTEGER PRIMARY KEY, target_language TEXT, tts_server_url TEXT, asr_server_url TEXT,
                ai_provider TEXT, ai_base_url TEXT, ai_api_key TEXT, embedding_model TEXT, active_model TEXT,
                huggingface_token TEXT, litert_accelerator TEXT, litert_max_tokens INTEGER,
                target_programming_language TEXT, coding_theme_category TEXT, active_vrm TEXT,
                supertonic_voice_style TEXT, tts_provider TEXT
            );
            INSERT INTO settings VALUES (
                1, 'French', '', '', 'custom', 'http://{address}/v1', 'test-key',
                'embed', 'model-a', '', 'Auto', 1024, 'python', 'All', 'avatar.vrm', 'voice_styles/F1.json', 'auto'
            );
        "#)).unwrap();
        ...
        assert_eq!(adapter.embedding("text".into(), "embed".into()).await.unwrap().len(), 768);
    }
}
